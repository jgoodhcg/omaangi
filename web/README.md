...
* [`src/umeng/web/`](src/umeng/web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/umeng/web/core.cljs): contains the SPA entry point, `init`
* [`test/umeng/web/`](test/umeng/web/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `umeng.web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/umeng/web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns umeng.web.example
  (:require [umeng.web.config :as config])
